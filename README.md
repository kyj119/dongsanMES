...
- **현재 상태**: ✅ **Day 4 완료** - 우선순위 시스템 및 작업 진행 정보 대시보드
...
### ✅ Day 3: 주문서 수정 및 현장 대시보드 개선
- [x] 주문서 수정 페이지 품목 수정 기능 (체크박스 선택/삭제)
- [x] 현장 대시보드 대폭 UI 개선 (큰 카드 형태)
- [x] 거래처 정보, 출고 정보, 품목 정보 상세 표시
- [x] 배송 방법 및 결제 방법 뱃지 표시
- [x] 디자인 파일 링크 클릭으로 탐색기에서 열기
- [x] 30초 자동 새로고침 기능
- [x] 긴급 작업(오늘 출고) 표시
- [x] 검색에 거래처명 추가
- [x] SQLite TimeSpan ORDER BY 에러 수정

### ✅ Day 4: EPS 썸네일 및 우선순위 시스템
- [x] **EPS 파일 자동 썸네일 생성** (Ghostscript 연동)
  - 업로드 시 자동으로 JPG 썸네일 생성 (원본 비율 유지)
  - 카드/주문서 상세에서 썸네일 미리보기 및 확대 모달
  - 파일 열기 버튼으로 원본 EPS 탐색기에서 열기
  - 썸네일 없을 시 (썸네일 없음) 표시
- [x] **배송방법별 우선순위 자동 시스템**
  - 퀵/용차: 우선순위 1 (최우선)
  - 대신택배/대신화물: 우선순위 2 (높음, 16:00 기본)
  - 한진택배: 우선순위 3 (보통, 18:00 기본)
  - 방문수령/직접배송: 우선순위 4 (낮음)
  - 우선순위는 자동 설정 (UI에서 숨김, 관리자만 접근)
- [x] **작업 진행 정보 대시보드**
  - 진행률 표시: 대기 0%, 작업중 50%, 완료 100%
  - 출고까지 남은 시간 표시
  - 알림 레벨: 여유(4시간+) / 주의(4시간 이내) / 긴급(2시간 이내)
  - 프로그레스 바 시각화
  - 완료된 카드는 진행 정보 미표시
- [x] **대시보드 정렬 개선**
  - 정렬 순서: 우선순위 ASC → 출고일시 ASC → 카드번호 ASC
  - 가장 빨리 처리해야 할 작업 최우선 표시

### 🚀 Day 5 예정: 주문서 관리 고급 기능
- [ ] 같은 거래처의 같은 품목 입력 시 최근 금액 자동 입력
- [ ] 주문서 목록 검색 기능 개선 (날짜 필터, 상태 필터)
- [ ] 규격 비례 전체 소요 시간 계산 시스템
- [ ] 통계 및 리포트 기능
...
### [v0.3] - 2026-02-05 (Day 3)
**추가:**
- 주문서 수정 페이지 품목 수정 기능 (체크박스 선택/삭제)
- 현장 대시보드 대폭 UI 개선 (큰 카드 형태)
- 디자인 파일 링크 클릭으로 탐색기에서 열기 (`POST /api/files/open`)
- 30초 자동 새로고침 기능
- 긴급 작업(오늘 출고) 표시
- 배송 방법 및 결제 방법 뱃지 표시

**수정:**
- SQLite TimeSpan ORDER BY 에러 수정 (메모리 정렬로 변경)
- 품목 검증 오류 수정 (자동완성 대응)
- 검색에 거래처명 추가
...
**마지막 업데이트**: 2026-02-05  
**현재 진행도**: ✅ Day 2 완료 (Collector 개발 + 문서화)
